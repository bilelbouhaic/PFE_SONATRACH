...
namespace api.Controllers
{
    ...
    public class KpiController : ControllerBase
    {
        ...
        public class CubeDataRow
        {
            ...
            public double Valeur1 { get; set; }
            public double Valeur2 { get; set; }
        }
        ...
        public IActionResult GetCubeData(string date)
        {
            ...
            string mdxQuery = @"
                 SELECT NON EMPTY {
                     [Measures].[Somme Taxe Mois], [Measures].[Varible Taux Taxe Moyen],
                      [Measures].[Maximum Ratio Rentabilité] } ON COLUMNS,
                  NON EMPTY { ([D Date 2].[Id Date].[Id Date].["+date+@"] ) } ON ROWS 
                  FROM [ED1 Fiscalite] 
            ";
                ...
                using (AdomdCommand cmd = new AdomdCommand(mdxQuery, conn))
                {
                    using (AdomdDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            {
                                Valeur = reader.GetDouble(1),
                                Valeur1 = reader.GetDouble(2),
                                Valeur2 = reader.GetDouble(3)
                            };
