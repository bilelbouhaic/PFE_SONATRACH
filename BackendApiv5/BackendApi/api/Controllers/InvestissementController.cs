...
namespace api.Controllers
{
    ...
    [ApiController]
    public class InvestissementController : ControllerBase
    {
        // Data context for interacting with the database
        private readonly ApplicationDBContext _context;

        // Constructor for dependency injection of the data context
        public InvestissementController (ApplicationDBContext context)
        {
            _context = context;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> PostMultipleInvest([FromBody] List<InvestissementDto> dtos)
        {
            ...
            var newInvests = new List<Investissement>();

            // Convert each DTO to a PrixBase entity and add to the list
            foreach (var dto in dtos)
            {
                var newInvest = new Investissement
                {
                    montantInvest = dto.montantInvest,
                    typeInvest = dto.typeInvest,
                    dateInvet = dto.dateInvet
                };

                newInvests.Add(newInvest);
            }

            // Add the list of new entities to the data context
            _context.investissements.AddRange(newInvests);

            // Save changes asynchronously
            await _context.SaveChangesAsync();

            // Return a response indicating successful creation of multiple entities
            return Ok(newInvests);  // Return the created objects with HTTP 200 OK
        }
